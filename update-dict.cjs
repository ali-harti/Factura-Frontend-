...
// Enhance walkDom to trim better and match exactly.
const walkDomRegex = /if \(text && dict\[text\]\) \{[\s\S]*?textNodes\.push\(\{ node, en: text, fr: dict\['How it works_section'\] \}\);\n\s*\}/;
const newWalkDom = `
        const strippedText = text.replace(/\\s+/g, ' '); // normalize spaces
        
        // Find best match in dict
        let matchedFr = dict[strippedText];
        
        // Special case for elements that have child elements inside (like pricing cards with svg icons)
        // In our naive implementation, textNodes are purely text nodes.
        if (matchedFr) {
...
`;
html = html.replace(walkDomRegex, newWalkDom);
