  ...
  return (
    <div className="auth-view" style={{ display: 'flex', opacity: 1, minHeight: '100vh', alignItems: 'center', justifyContent: 'center' }}>
      <div className="auth-card enter" id="auth-card">
        ...
        <div id="login-container" style={{ display: 'block', opacity: 1 }}>
          <h2 className="auth-title">Welcome back</h2>
          <p className="auth-sub">Sign in to continue to Factura</p>
          
          {error && (
            <div className="auth-error-banner" id="login-error" style={{ display: 'block' }}>
              {error}
            </div>
          )}
          ...
          <form id="login-form" onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <input type="email" id="login-email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@company.com" required />
            </div>
            <div className="form-group">
            ...
            <button type="submit" className="btn btn-primary auth-submit-btn" id="login-btn" disabled={loading}>
